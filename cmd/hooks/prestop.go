...
func checkActivePods(clientset kubernetes.Interface, nodeName string, allVolumesUnmounted chan struct{}) error {
	...
	if err != nil {
		klog.V(2).ErrorS(err, "failed to get podList")
		return nil
	}
		...
		if pod.Spec.NodeName == nodeName {
			for _, vol := range pod.Spec.Volumes {
				if vol.PersistentVolumeClaim != nil {
					klog.InfoS("checkActivePods: not ready to exit, found PVC associated with pod", "PVC", vol.PersistentVolumeClaim.ClaimName, "node", nodeName)
					return nil
				}
			}
		}
